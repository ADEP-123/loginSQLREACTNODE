import { postNewFuenteService, postNewIncomeService, postNewOutcomeService, postNewUserService } from "../services/postServices.js"
...
export const postNewFuenteController = async (req, res, next) => {
    const { user } = req.data.payload
    const { fuente,descripcion } = req.body
    try {
        const result = await postNewFuenteService(user, fuente,descripcion)
        res.status(200).send({ status: true, result })
    } catch (error) {
        res.status(500).send({ status: false, error })
    }
